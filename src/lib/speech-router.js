import { PRIMARY_ENGLISH_PIPER_VOICE_ID, speak, stopSpeaking } from './audio/tts.js'
...
export function resolveVoiceForSegment({ language, role, voiceId, settings = {} }) {
  const english = settings.english_voice_id || settings.piper_voice || PRIMARY_ENGLISH_PIPER_VOICE_ID
  const portuguese = settings.portuguese_explanation_voice_id || PORTUGUESE_FABIOLA_VOICE_ID
  const rate = role === 'explanation_pt' || String(language).startsWith('pt')
    ? (settings.portuguese_voice_rate || 1)
    : (settings.english_voice_rate || settings.tts_rate || 1)
...
export async function speakSegment({ text, language = 'en', role = 'exercise_en', voiceId = null, rate = null, settings = {}, interrupt = true } = {}) {
  ...
  const result = await speak(text, {
    rate: rate ?? routed.rate,
    voiceId: selected,
    requestedVoiceId: selected,
    language,
    role,
    interrupt,
  })
...
export async function speakFeedbackSequence(presentation, settings = {}) {
  ...
  const pt = presentation?.speech_segments?.find((segment) => segment.role === 'explanation_pt')
  const en = presentation?.speech_segments?.find((segment) => segment.role === 'correct_answer_en')
