...
test('V2 Settings exports, hard-deletes and collision-safely restores one profile', async ({ page, context }) => {
  ...
  expect(profilesAfterDelete.map((row) => row.profile_id)).not.toContain(PROFILE_A)
  expect(profilesAfterDelete.map((row) => row.profile_id)).toContain(PROFILE_B)
  ...
  expect(profilesAfterRestore.map((row) => row.profile_id).sort()).toEqual([PROFILE_A, PROFILE_B].sort())
